...
Public CI uses the minimal declarations under `tests/ue4ss_stubs` to compile the Windows adapter and verify `start_mod` and `uninstall_mod`. Those declarations are not shipped and do not claim UE4SS ABI compatibility. Production builds require an authorized, pinned RE-UE4SS checkout through `PALCENTER_UE4SS_ROOT` plus live PalServer validation.
